(async function () {
  ...
  function normalizeInternshipMatch(value) {
  ...
  function getEntriesPayloadMeta(entriesOrPayload) {
    if (Array.isArray(entriesOrPayload)) {
      return {
        entries: entriesOrPayload,
        defaultInternship:
          entriesOrPayload[0]?.internship ??
          entriesOrPayload[0]?.internshipName ??
          entriesOrPayload[0]?.internship_name ??
          ""
      };
    }

    if (entriesOrPayload && typeof entriesOrPayload === "object") {
      const nestedEntries = Array.isArray(entriesOrPayload.entries)
        ? entriesOrPayload.entries
        : Array.isArray(entriesOrPayload.data)
          ? entriesOrPayload.data
          : null;
      ...
      if (nestedEntries) {
        return {
          ...
          defaultInternship:
            entriesOrPayload.internship ??
            entriesOrPayload.internshipName ??
            entriesOrPayload.internship_name ??
            ""
        };
      }
    }

    throw new Error("data.json must be an array of entries or an object with an entries array");
  }

  async function loadEntries() {
    const importedEntries = await getImportedEntries();
    const payload = importedEntries.length ? importedEntries : await (async () => {
    ...
    const { entries, defaultInternship } = getEntriesPayloadMeta(payload);
    ...
    const normalized = entries.map((entry, index) => {
      ...
      const internship =
        entry?.internship ??
        entry?.internshipName ??
        entry?.internship_name ??
        defaultInternship ??
        "";
  ...
  function scoreInternshipOption(optionText, requestedInternship) {
    const requested = normalizeInternshipMatch(requestedInternship);
    const option = normalizeInternshipMatch(optionText);
    ...
    if (option.includes(requested) || requested.includes(option)) {
      return 80;
    }

    const requestedWords = requested.split(" ").filter(Boolean);
    const optionWords = new Set(option.split(" ").filter(Boolean));
    const sharedWords = requestedWords.filter((word) => optionWords.has(word)).length;
    return sharedWords ? Math.round((sharedWords / requestedWords.length) * 60) : 0;
  }

  function pickBestInternshipOption(options, requestedInternship) {
    if (!options.length) {
      return null;
    }

    if (!requestedInternship) {
      return options[0];
    }
    ...
    return options
      .map((option) => ({
        option,
        score: scoreInternshipOption(option.textContent || option.label || option.value || "", requestedInternship)
      }))
      .filter((entry) => entry.score > 0)
      .sort((left, right) => right.score - left.score)[0]?.option || null;
  }

  async function selectInternshipOption(dropdown, requestedInternship) {
    if (dropdown.tagName.toLowerCase() === "select") {
      ...
      const selectedOption = pickBestInternshipOption(options, requestedInternship);
      if (!selectedOption) {
        throw new Error(`Internship "${requestedInternship}" was not found`);
      }
    ...
    if (typedOption && (!requestedInternship || scoreInternshipOption(typedOption, requestedInternship) > 0)) {
      dispatchInputEvents(dropdown);
      return;
    }

    throw new Error(requestedInternship ? `Internship "${requestedInternship}" was not found` : "No internship options found");
  }
  ...
  async function fillPage1(entry) {
    ...
    try {
      await retry(() => selectInternshipOption(internshipDropdown, entry.internship), 3);
    } catch (error) {
      if (error.message === "No internship options found" || error.message.includes("was not found")) {
        await requestRecoveryReload(
          error.message === "No internship options found"
            ? `Internship options did not load for ${entry.date}. Reloading once to recover.`
            : `${error.message}. Reloading once to recover.`,
          "reload_internship"
        );
      }
